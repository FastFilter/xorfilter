...
// Todo: delete
func populateBinaryFuse8Alt(keys []uint64) (*BinaryFuse8, error) {
	size := uint32(len(keys))
	filter := &BinaryFuse8{}
	filter.initializeParameters(size)
	rngcounter := uint64(1)
	...
	for true {
		...
		for Qsize > 0 {
			...
			H[index1].count--
			if H[index1].count == 1 {
				Q[Qsize].index = index1
				Q[Qsize].hash = H[index1].xormask
				Qsize++
			}
			H[index2].xormask ^= hash
			H[index2].count--
			if H[index2].count == 1 {
				Q[Qsize].index = index2
				Q[Qsize].hash = H[index2].xormask
				Qsize++
			}
			H[index3].xormask ^= hash
			H[index3].count--
			if H[index3].count == 1 {
				Q[Qsize].index = index3
				Q[Qsize].hash = H[index3].xormask
				Qsize++
			}
		}
	...
	for stacksize > 0 {
		stacksize--
		ki := stack[stacksize]
		index1, index2, index3 := filter.getHashFromHash(ki.hash)
		fp := uint8(fingerprint(ki.hash))
		switch ki.index {
		case index1:
			fp ^= filter.Fingerprints[index2] ^ filter.Fingerprints[index3]
		case index2:
			fp ^= filter.Fingerprints[index1] ^ filter.Fingerprints[index3]
		default:
			fp ^= filter.Fingerprints[index1] ^ filter.Fingerprints[index2]
		}
